...
static constexpr uint32_t CF_ERROR = 0xF40003UL;  // READ: Error register
static constexpr uint32_t CF_FEATURES = 0xF40003UL;  // WRITE: Features register
...
static constexpr uint32_t CF_STATUS = 0xF4000FUL;  // READ: Status register
static constexpr uint32_t CF_COMMAND = 0xF4000FUL;  // WRITE: Command register
...
static constexpr uint32_t DUART_MR1A = 0xF80001UL;  // RW: Channel A mode register 1
static constexpr uint32_t DUART_MR2A = 0xF80001UL;  // RW: Channel A mode register 2
...
static constexpr uint32_t DUART_RBA = 0xF80007UL;  // READ: Channel A receiver buffer
static constexpr uint32_t DUART_TBA = 0xF80007UL;  // WRITE: Channel A transmitter buffer
static constexpr uint32_t DUART_IPCR = 0xF80009UL;  // READ: Input port change register
...
static constexpr uint32_t DUART_MR1B = 0xF80011UL;  // RW: Channel B mode register 1
static constexpr uint32_t DUART_MR2B = 0xF80011UL;  // RW: Channel B mode register 2
...
static constexpr uint32_t DUART_RBB = 0xF80017UL;  // READ: Channel B receiver buffer
static constexpr uint32_t DUART_TBB = 0xF80017UL;  // WRITE: Channel B transmitter buffer
...
static constexpr uint32_t DUART_STARTCC = 0xF8001DUL;  // READ: Start counter/timer command (read)
static constexpr uint32_t DUART_OPR_SET = 0xF8001DUL;  // WRITE: Output port bit set (sets OPR bits; those OP pins go LOW)
static constexpr uint32_t DUART_STOPCC = 0xF8001FUL;  // READ: Stop counter/timer command (read)
static constexpr uint32_t DUART_OPR_CLR = 0xF8001FUL;  // WRITE: Output port bit reset (clears OPR bits; those OP pins go HIGH)
...
static constexpr uint32_t DUART_IP_DCD_B = 0x10U;
static constexpr uint32_t DUART_IP_RI_B = 0x20U;
...
static constexpr unsigned SYS_WRITE = 1U;  // write(fd, buf, len) to the console or a file.
static constexpr unsigned SYS_READ = 2U;  // read(fd, buf, len) from the console or a file.
static constexpr unsigned SYS_OPEN = 3U;  // open(path, flags, mode); returns a file descriptor.
static constexpr unsigned SYS_CLOSE = 4U;  // close(fd).
static constexpr unsigned SYS_LSEEK = 5U;  // lseek(fd, offset, whence).
static constexpr unsigned SYS_FSTAT = 6U;  // fstat(fd, stat buffer).
static constexpr unsigned SYS_ISATTY = 7U;  // isatty(fd).
static constexpr unsigned SYS_STAT = 8U;  // stat(path, stat buffer).
...
static constexpr unsigned SYS_READDIR = 10U;  // readdir(path, index, entry buffer) fills one directory entry and returns 0, or 1 past the end.
