...
* **Debug headers — LA access + "PCB design failed" backup:** two 2×16 shrouded 0.1" box headers laid out to **mirror the LogicAnalyzer V2 board's own 2×16 input connector** (24 channels + 2 GND + EXT_VREF + EXT_TRIG + the analyzer's own 5 V and 3V3 pairs; straight 32-way IDC ribbon, Griffin pin n = analyzer pin n), so the two on-hand 5 V-tolerant analyzers plug straight on and daisy-chain via their own 3-pin chain connectors into one synchronized 48-channel capture.  Channel-to-pin mapping taken from the gusmanb LogicAnalyzerV2 JITX design (board/place_headers.py, DEBUG_HEADER sheet); the level shifters and the 3V3/5V/ext VREF switch live on the analyzer board, Griffin only feeds EXT_VREF from +5 V.
...
- [x] Debug headers: 2× 2×16 shrouded box headers (board.pretty IDC-Header_2x16) mirroring the gusmanb LogicAnalyzer V2 input connector, 32-way IDC ribbon straight through — DEBUG_HEADER sheet; header 1 = D0–D15 + ~AS/~UDS/~LDS/R/~W/~DTACK + ~RESET/~HALT (23 — spare-channel candidates: audio line clock or paddle DUMP), header 2 = A1–A23 + SYSCLK (via R32 100R series tap, local net SYSCLK_TAP — never a bare stub on the clock net); pin 3 EXT_VREF from +5 V, the analyzer's 5 V/3V3 pins (5–8) and EXT_TRIG (4) open
...
- [ ] series axial footprint at the driver output on every 7200 /RE and /W line, all three streams, stuffed 0R (33–47 Ω if a strobe rings on the bench; Rev 1's ringing came from breadboard jumpers with no return plane, see griffin.log 2026-09-12); no q8_toggle in Rev 2
