...
* **PORTS ATF1508 — the fourth CPLD, 0xFC0000:** PS/2 **mouse** frame engine (same design as GLUE's keyboard one, same register offsets so one firmware driver serves both), both **joystick** ports read as bytes, both **paddle** counters + the dump control, and the **audio FIFO pop** strobe with the 7200 ~HF IRQ latch.  It carries no bus timing of its own: GLUE hands it a cycle-qualified ~PORTS_SELECT and answers DTACK, exactly as for CF.  Its time bases are two nets from TIMING — **PADDLE_TICK and AUDIO_TICK**, square waves toggling once per line, synchronized on arrival and counted on falling edges (15.734 kHz events for both).  No prescaler, no programmable divider, anywhere.  cpld/ports/ports.v takes its register decode from the generated griffin.yml defines and its pinout is frozen in its `//PIN:` block; it does not use `xor_synthesis` (measured to make no difference — griffin.log).  See pcbv2-ports-design.md.  This is what retires the 2× 74HCT245 and 2× 74HC590.
* **Audio - FIFO** (replaces the Rev 1 '373 latch): 2× 7200 FIFO (256×9; 256 pairs = 16 ms of buffer, ~HF at 128 = ~123 Hz refill, cascadable via XI/XO if depth is ever wanted) + 2× 8-bit R2R DAC, stereo (L = D[15:8], R = D[7:0] straight to the FIFOs).  ~W is ENGINE's AUDIO_FIFO_W descriptor strobe: the display list deposits sample words by DMA and there is no CPU-mapped write path.  **PORTS** generates ~R once per AUDIO_TICK event from TIMING, and ~HF goes to PORTS, which latches it into ~PORTS_IRQ (level 2) as the underrun warning, acked with PORTS AUDIO_CONTROL; baseline ~15.7 kS/s; no mono packing.
* **Interrupts (autovector):** vsync 6 (TIMING's dedicated nVSYNC_IRQ output, pin 31, to GLUE pin 65 — polarity fixed regardless of raster mode — edge-latched there, W1C via GLUE VSYNC_CLEAR; TIMING has no bus to host the ack), DUART 5, PS/2 keyboard 4, ENGINE 3, PORTS 2 (mouse + audio half-full, wire-ORed inside the CPLD).  ENGINE still ties ~ENGINE_IRQ high, so level 3 is inert today; the GLUE pin and the net exist so enabling it later is a bitfile change, not a respin.
...
- [ ] Audio: 2× 7200 FIFO + 2× R2R DAC on the board (replaces the Rev 1 '373 + write-only ~AUDIO_LE); ~W from ENGINE's AUDIO_FIFO_W strobe pin, **~R and the ~HF IRQ from PORTS**; /RS source open (griffin.yml issues)
...
- [ ] PORTS: ATF1508AS PLCC84 socket + JTAG chain; nets ~PORTS_SELECT / ~PORTS_IRQ / A4-A1 / ~LDS / R~W / D7-D0 to GLUE and the bus, **PADDLE_TICK (TIMING 17 → PORTS 44) and AUDIO_TICK (TIMING 18 → PORTS 45)** as its only time bases, ~R (nFIFO_RE) and ~HF to the 7200 pair, mouse PS/2, both DE-9s, PADDLE_DUMP to the FET gates — pin numbers from the frozen `//PIN:` block in cpld/ports/ports.v
...
- [ ] Audio: ~W from ENGINE's AUDIO_FIFO_W strobe, ~R and the ~HF IRQ from PORTS
